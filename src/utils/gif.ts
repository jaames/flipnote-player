...
export const gifUrlFromNoteAnimation = (note: Flipnote) => {
  return GifImage.fromFlipnote(note).getUrl()
} ;

export const gifUrlFromArrayBuffer = (data: ArrayBuffer) => URL.createObjectURL(new Blob([data], { type: GIF_MIME }));

export const gifUrlRevoke = (url: string) => { console.log(`revoked ${url}`); if (url && url.startsWith('blob:')) URL.revokeObjectURL(url); }